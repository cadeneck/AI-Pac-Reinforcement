...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    action_value = sum(prob * (self.mdp.getReward(state, action, nextState) +
                                               self.discount * self.values[nextState])
                                        for nextState, prob in self.mdp.getTransitionStatesAndProbs(state, action))
                    best_action_value = max(best_action_value, action_value)

                updated_values[state] = best_action_value
    ...
    def computeActionFromValues(self, state):
        ...
        return max(legal_actions, key=lambda action: self.computeQValueFromValues(state, action))
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def find_predecessors(self):
        """
        Find predecessors for each state.
        """
